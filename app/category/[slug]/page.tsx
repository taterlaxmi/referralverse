...
export default async function CategoryPage({ params }: Props) {
  ...
  const categoryGraphSchema = {
    ...
    "@graph": [
      ...
      itemListSchema,
      ...(categoryContent[categoryName as Category]?.faqs ? [
        schemaUtils.getCategoryFaqSchema(`https://referralverse.in/category/${slug}`, categoryContent[categoryName as Category]!.faqs)
      ] : [])
    ]
  };

  const seoData = categoryContent[categoryName as Category];
        ...
        <div className="container mx-auto px-4 py-8">
          ...
          <div className="text-center my-8">
            <h1 className="text-4xl md:text-5xl font-extrabold text-gray-800 tracking-tight">
              {seoData?.headerTitle ? `${seoData.headerTitle} (${monthYear})` : `Best ${categoryName} Referral Codes ${monthYear}`}
            </h1>
            {seoData?.headerDescription ? (
          ...
          {seoData?.faqs && (
            <div className="max-w-4xl mx-auto py-12">
              <FAQSection 
                faq={seoData.faqs} 
                heading={`${categoryName} Referral FAQs`}
                subheading={`Common questions about ${categoryName} offers`}
              />
            </div>
          )}
        </div>
