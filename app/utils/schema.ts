...
export function getWebSiteSchema() {
  return {
    ...
    name: "ReferralVerse",
    description:
      "Find top referral offers, coupons and promo codes curated for travel, finance, food and lifestyle.",
...
export function getWebPageSchema(post: Post, relatedPosts: Post[] = []) {
  ...
  if (relatedPosts.length > 0) {
    baseSchema.relatedLink = relatedPosts.map(
      (rp) => `https://referralverse.in/${rp.slug}`,
    );
  }
    ...
    "@graph": [
      ...
      },
      {
        ...
        name: query
          ...
          : "Latest Referral Offers",
        description: query
          ? `Referral offers matching your search for ${query}`
          : "A collection of the best referral offers and coupons.",
